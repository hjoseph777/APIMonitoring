...
// Run initial database seed if seed endpoints are missing
try {
  const endpoints = DatabaseService.getEndpoints()
  const hasSeeds = endpoints.some((e) => e.id.startsWith('seed-'))
  if (!hasSeeds) {
    console.log('Seeding initial demo database endpoints, alerts, and logs...')
    const NOW = new Date()
